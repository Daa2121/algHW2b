...
def heapify(arr, parent, upper):
    while (True):
        ...
        if max(l, r) < upper:
            #if the parent is smaller than the children then we are done
            if arr[parent] <=  min(arr[l], arr[r]): break
            #check which node is smaller and swap it with the parent then set the swaped value to be the new parent
            elif arr[l] > arr[r]:
                swap(arr, parent, r)
                parent = r
            else:
        ...
        #left child node exists 
            ...
            if arr[l] < arr[parent]:
                swap(arr, parent, l)
                parent = l
            #if the child isn't smaller than the parent we are done
            else: break
        elif r < upper:
            if arr[r] < arr[parent]:
                swap(arr, parent, r)
...
def heapsort(arr):
    ...
    for upper in range(len(arr)-1, 0, -1):
        #the very first element in a min heap is always going to be the smallest element in the heap
        ...
        swap(arr, 0, upper)
        heapify(arr, 0, upper)
    #after sorting simply reseverse the array to have it in ascending order
    return arr[::-1]
